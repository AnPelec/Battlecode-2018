...
def get_free_location(unit):
	'''game_map = bc.GameMap
	map_of_mars = game_map.mars_map
	print('O TUPOS', type(map_of_mars))
...
def launch_rocket(unit):
	if not unit.location.is_on_planet(bc.Planet.Earth):
		return
	...
	try:
		for current_unit in gc.my_units():
			...
			if current_unit.unit_type == bc.UnitType.Worker:
				if gc.round()%7 == 0:
					random_get_karbonite(current_unit)
				if gc.round() < 10:
					...
					if (gc.round()%120 == 0) or (can_be_loaded(current_unit)):
						complete_build(current_unit, bc.UnitType.Rocket)
						build_blueprint(current_unit, bc.UnitType.Rocket)
					if gc.round()%120 == 30:
						complete_build(current_unit, bc.UnitType.Factory)
						build_blueprint(current_unit, bc.UnitType.Factory)
					if (gc.round()%3 == 0) and (not can_be_loaded(current_unit)):
						random_move(current_unit)
			...
			if current_unit.unit_type == bc.UnitType.Rocket:
				if current_unit.location.is_on_planet(bc.Planet.Earth):
